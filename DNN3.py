...
class DNN:
    ...
    def build(self):
        ...
        logits = dense(dropout_layer1_2, self.cate_num, activation=None, name='output_category')
        self.outputs_cate = tf.nn.softmax(logits)

        # 网络结构(二)
        dropout_layer2 = dropout(self.x_, keep_prob=self.keep_prob2, name='dropout_layer2')
        inputs = tf.concat([self.outputs_cate, dropout_layer2], axis=1)
        layer2_1 = dense_bn_activation(inputs, 15, tf.nn.tanh, name='layer2_1')
        layer2_2 = dense_bn_activation(layer2_1, 10, tf.nn.tanh, name='layer2_2')
        outputs_norm = dense(layer2_2, 1, tf.nn.sigmoid, name='output_y')
        self.outputs_y = tf.round(100 * outputs_norm)
        ...
        with tf.name_scope('optimizer'):
            ...
            self.loss = 2*loss_y + loss_cate + sum(reg_losses)
        ...
        for e in tqdm(range(self.epoch), 'epoch'):
            ...
            for _ in range(self.batch_num):
                ...
                feed_dict = {self.x_: self.batch_x, self.cate: self.batch_cate, self.y_: self.batch_y,
                             self.learning_rate: self.lr, self.keep_prob: 0.5, self.keep_prob2: 0.1}
                ...
                feed_dict = {self.x_: self.X_val, self.cate: self.cate_val, self.y_: self.y_val,
                             self.keep_prob: 1.0, self.keep_prob2: 1.0}
    ...
    def evaluate(self, sess, X_test, y_test):
        ...
        feed_dict = {self.x_: X_test, self.keep_prob: 1.0, self.keep_prob2: 1.0}
